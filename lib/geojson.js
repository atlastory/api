...
exports.import = function(options) {
    ...
    if (isNaN(period) || typeof type !== 'number')
        return Q.reject(util.err("'period' or 'type' ID missing"));

    var sql = exports.getImportSql(options)+'';

    if (/--\sError/.test(sql)) return Q.reject(util.err(sql, 'generating import sql'));
...
exports.export = function(options) {
    ...
    if ((isNaN(period) && isNaN(year)) || !_.isArray(type)) return Q.reject(util.err("'period', 'year' or 'type' invalid", "exporting GeoJSON"));
    ...
    async.parallel({ properties: function(done) {
        Shape.getData({ period: period, year: year, type: type }).select(cols).run(done);
    }, nodes: function(done) {
        Shape.getNodes({ period: period, year: year, type: type, box: bbox }).run(done);
    }}, function(err, res) {
