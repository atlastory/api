...
exports.import = function(options, callback) {
    ...
    var geojson = options.geojson,
        ...
        duplicate = (typeof duplicate === 'boolean') ? duplicate : true,
        changeset = util.createHash(),
        each, properties, changesetId;

    if (!period || !type) return callback("geojson#import: 'period' or 'type' ID missing");
    ...
    each(geojson.features, function(feature, finishFeature) {
        ...
        // TODO: if duplicate, check if 'name' already exists,
        // if so, add period# to that shape's period array
        ...
        // Add feature to NWS structure
            ...
            changesetId = cid;
            data.changeset_id = geom.changeset_id = cid;
        ...
        if (err) callback(err);
        else callback(null, changesetId);
    });
};
