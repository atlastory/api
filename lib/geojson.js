...
exports.import = function(options, callback) {
    ...
    function checkExistingShape(data, callback) {
        Shape.select('id').where({
            ...
            else if (shapes.length > 0) callback(shapes[0].id);
            else callback(null);
        });
    }
    ...
    function addShape(data, geom, callback) {
        ...
        async.waterfall([function(next) {
            ...
            case "Point":
            case "MultiPoint":
                Node.create(geom.coordinates, changeset, function(err, ids) {
                    if (err) next("Point | MultiPoint > "+err);
                    else {
                        nws.addNodes(ids);
                        Shape.finish(data, nws.getShapes(), next);
                    }
                });
                break;

            case "LineString":
                Way.create({ coordinates: geom.coordinates, changeset: changeset }, function(err, id) {
                    if (err) next("LineString > "+err);
                    else {
                        nws.addWays(id);
                        Shape.finish(data, nws.getShapes(), next);
                    }
            ...
            case "MultiLineString":
                geom = geom.coordinates.map(function(way) {
                    return { coordinates: way, changeset: changeset };
                });
                async.map(geom, Way.create, function(err, ids) {
                    if (err) next("Polygon | MultiLineString > "+err);
                    else {
                        nws.addWays(ids);
                        Shape.finish(data, nws.getShapes(), next);
                    }
            ...
            case "MultiPolygon":
                geom = _.flatten(geom.coordinates.map(function(poly) {
                    return poly.map(function(innerPoly) {
                        return { coordinates: innerPoly, changeset: changeset };
                    });
                }));
                async.map(geom, Way.create, function(err, ids) {
                    if (err) next("MultiPolygon > "+err);
                    else {
                        nws.addWays(ids);
                        Shape.finish(data, nws.getShapes(), next);
                    }
