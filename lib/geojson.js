...
exports.import = function(options, callback) {
    ...
    function addShape(data, geom, callback) {
        ...
        async.waterfall([function(next) {
            ...
            function getPolyWays() {
                ...
                return _.flatten(coords.map(function(poly) {
                    return poly.map(function(way, i) {
                        var role = (i != 0) ? 'inner' : 'outer';
                        return { coords: way, role: role };
                    });
                }));
            }

            function finishShape(err, ids) {
            ...
            case "Point":
            case "MultiPoint":
                Node.create(coords, nodeData, function(err, ids) {
                    if (err) next(geom.type + ' > ' + err);
                    else {
                        nws.addNodes(ids);
                        Shape.finish(data, nws.getShapes(), next);
            ...
            case "LineString":
                Way.create(coords, nodeData, finishShape);
                break;

            case "Polygon":
            case "MultiLineString":
                async.map(getPolyWays(), function(way, nextWay) {
                    Way.create(way.coords, nodeData, nextWay);
                }, finishShape);
                break;
            ...
            addDirective({
                ...
                // TODO: is geometry necessary for 'add' directive?
                // geometry: geom.type
