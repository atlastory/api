 ...
 *        {number} [.id] existing changeset ID || user
 *        {number} [.user_id=0]
 *        {string} [.message='Import of GeoJSON']
 * @returns {number} changeset ID
 */
exports.import = function(options, changeset) {
    ...
    if (!util.isBigint(period)) return err.reject("'period' ID missing or invalid");
    if (!util.isBigint(type)) return err.reject("'type' ID missing or invalid");

    var sql = exports.getImportSql(options, changeset)+'';
 ...
 * @param {object} changeset (same as #import() above)
