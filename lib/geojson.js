...
exports.import = function(options) {
    ...
    if (isNaN(period) || typeof type !== 'number')
        return Q.reject(err.new("'period' or 'type' ID missing"));

    var sql = exports.getImportSql(options)+'';

    if (/--\sError/.test(sql)) return Q.reject(err.new(sql, 'generating import sql'));
...
exports.export = function(options) {
    ...
    if ((!util.isBigint(period) && isNaN(year)) || !_.isArray(type))
        return Q.reject(err.new("'period', 'year' or 'type' invalid", "exporting GeoJSON"));
    ...
    return Q.all([
        Shape.getData({ period: period, year: year, type: type }).select(cols),
        Shape.getNodes({ period: period, year: year, type: type, box: bbox })
    ]).then(function(res) {
        geoJSON.features = _(res[1]).groupBy(function(node) {
            return node.shape;
        }).map(function(shape, id) {
            var props = _.find(res[0], { 'id': id });
            return {
                "type": "Feature",
