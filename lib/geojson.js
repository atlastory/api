...
exports.import = function(options, callback) {
     ...
     * execute   BOOLEAN (default true) false = returns raw SQL
     */
    ...
    var geojson = options.geojson,
        ...
        execute = _.isBoolean(options.execute) ? options.execute : true;
    ...
    async.each(geojson.features, importFeature, function(err) {
        if (!execute) return callback(null, sql);

        pg.query(sql, function(err, cs) {
            if (err) return util.err({message:'error executing SQL', details:err});
            callback(null, _.compact(_.pluck(cs, 'changeset_id'))[0]);
        });
    });
    ...
    function addShapeSql(feature, callback) {
        ...
        if (/Point/.test(geom.type)) {
            ...
            queue.add(Shape.Relation.insert(coords).returning('LASTVAL()'))
                 .add(Temp.insert({ shape_id: [['LASTVAL()']] }));
