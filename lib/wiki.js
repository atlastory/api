 ...
 * @param {{message: string, directives: Directive[]}} changeset
 */
exports.updateMessage = function(id, changeset) {
    return Changeset.find(id).thenOne(function(cs) {
        if (!cs) return err.reject('Changeset #'+id+' not found');
        if (cs.status == 'done') {
            return err.reject('Changeset #'+id+' already commited');
        } else if (cs.message === '' && !changeset.message) {
            return err.reject('Changeset needs a commit message');
        } else if (changeset.message) {
            return Changeset.update({ message: changeset.message });
        } else return true;
    });
};

// Update Changeset status
exports.updateStatus = function(id, allGood) {
    return Changeset.update(id, {
        status: allGood ? 'done' : 'failed',
        finished_at: allGood ? new Date() : null
    });
};
...
exports.commit = function(id, changeset) {
    ...
    var completed, allGood;

    // Get changeset & update commit message
    return exports.updateMessage(id, changeset)
    // Run directives
    .then(function() {
        // Sort into Nodes, Ways, Shapes
        directives = _.flatten([
    ...
    .then(function(newDirectives) {
        ...
        completed = _.where(directives, { success: true });
        allGood = _.every(directives, { success: true });
        if (_.isEmpty(completed)) return true;

        // For now, record all directives, completed or not
        // return Changeset.Directive.create(id, completed);
        return Changeset.Directive.create(id, directives);
    }).then(function() {
        return exports.updateStatus(id, allGood);
    }).then(function() {
...
exports.mapToResponse = function(d) {
    ...
    var directive = !d.created_at ? Changeset.Directive.new(d) : d;
    ...
    } else if (d.success || d.created_at) {
        res.status = 'success';
    } else {
        res.message = d.message;
    }
...
exports.parse = function(directives) {
    ...
    return Q(directives).mapSeries(function(directive) {
        return exports.checkConflict(directive).then(function(inConflict) {
            if (inConflict) {
                directive.inConflict = true;
                return directive;
            }
            return diff.run(directive);
        });
    });
};
